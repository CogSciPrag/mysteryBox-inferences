...
marty2022_top6 %>% write_csv("../../results/03_tidy/martyetal2022_tidy_top6models.csv")
```
...
controls_all_wide_byStudy <- controls_all %>% select(-item_prob_accuracy, -better_metric) %>%
  mutate(wta_accuracy = round(wta_accuracy, 2)) %>%
  pivot_wider(names_from = c(Study), names_sep = "_", values_from = wta_accuracy)
controls_all_wide_byStudy <- controls_all_wide_byStudy %>%
  mutate(average_accuracy = 1/3*(`Deganoetal_2024`+ `Martyetal_2023`+ `Martyetal_2022`))

controls_all_wide_byModel <- controls_all %>% select(-item_prob_accuracy, -better_metric) %>%
  mutate(wta_accuracy = round(wta_accuracy, 2)) %>%
  pivot_wider(names_from = c(model), values_from = wta_accuracy)
#controls_all_wide %>% write_csv("../../results/02_processed_all/control_acc_all.csv")
#controls_all_wide_byStudy %>% write_csv("../../results/02_processed_all/control_acc_byStudy.csv")
#controls_all_wide_byModel %>% write_csv("../../results/02_processed_all/control_acc_byModel.csv")
```
...
degano_filtered <- degano2024_processed %>%
  filter( ((Experiment == "Exp_1") & (model == "phi-2")) | 
          ((Experiment == "Exp_1") & (model == "Llama-2-13b-hf")) |
            ((Experiment == "Exp_2") & (model == "phi-2")) | 
          ((Experiment == "Exp_2") & (model == "Llama-2-13b-hf")) |
            ((Experiment == "Exp_3") & (model == "text-davinci-003")) | 
          ((Experiment == "Exp_3") & (model == "Mixtral-8x7B-Instruct-v0.1")) |
